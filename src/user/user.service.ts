...
export class UserService {
  ...
  async createUserAndReturnId(body: CreateUserWithRole): Promise<number> {
    ...
    const userResult = await this.dbService.query(`INSERT INTO ${this.tableName} (name, email, password, role_id) VALUES (?, ?, ?, ?) RETURNING id`, [
      name,
      email,
      hashedPassword,
      role_id,
  ...
  async updateUser(user_id: number, body: CreateUserWithRole) {
    ...
    return this.dbService.run(`UPDATE ${this.tableName} SET name = ?, email = ?, password = ?, role_id = ? WHERE id = ?`, [name, email, password, role_id, user_id]);
  }

  async deleteUser(user_id: number) {
    return this.dbService.run(`DELETE FROM ${this.tableName} WHERE id = ?`, [user_id]);
  }
}
