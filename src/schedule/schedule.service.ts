...
export class ScheduleService {
  ...
  async create(body: CreateScheduleDto) {
    ...
    return this.dbService.run(`INSERT INTO ${this.tableName} (class_id, day_of_week) VALUES (?, ?)`, [class_id, day_of_week]);
  }

  async update(schedule_id: number, body: CreateScheduleDto) {
    const { class_id, day_of_week } = body;
    return this.dbService.run(`UPDATE ${this.tableName} SET class_id = ?, day_of_week = ? WHERE id = ?`, [class_id, day_of_week, schedule_id]);
  }

  async delete(schedule_id: number) {
    return this.dbService.run(`DELETE FROM ${this.tableName} WHERE id = ?`, [schedule_id]);
  }
}
