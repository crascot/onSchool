...
export class TaskService {
  ...
  async getOne(task_id: number): Promise<TaskType> {
    const result = await this.dbService.query(`SELECT * FROM ${this.tableName} WHERE id = ?`, [task_id]);
  ...
  async create(body: CreateTaskDto) {
    const { task, type, status, end_at, class_id, lesson_id, student_id } = body;
    ...
    return this.dbService.run(
      `UPDATE
			...
			SET
				...
				student_id = ?
			WHERE id = ?`,
      [task, type, status, end_at, class_id, lesson_id, student_id, task_id],
    );
  }

  async delete(task_id: number): Promise<void> {
    return this.dbService.run(`DELETE FROM ${this.tableName} WHERE id = ?`, [task_id]);
  }
}
